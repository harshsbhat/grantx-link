...
export default function Home() {
  ...
  const handleCopyCurl = () => {
    const curlCommand = `curl -X POST http://localhost:3001/api/shortenUrl \n-H "Content-Type: application/json" \n-d '{"originalUrl": "${originalUrl}", "apiKey": "${apiKey}"}'`;
    ...
    <main className="flex min-h-screen items-center justify-center p-6 bg-black">
      ...
      >
        <div className="bg-neutral-900 p-6 rounded-lg max-w-lg w-full">
          ...
          <pre className="bg-neutral-800 p-3 rounded mb-4 whitespace-pre-wrap break-all text-white">{`curl -X POST http://localhost:3001/api/createUrl \n-H "Content-Type: application/json" \n-d '{"originalUrl": "${originalUrl}", "apiKey": "${apiKey}"}'`}</pre>
          <button
